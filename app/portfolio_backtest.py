...
def download_prices(tickers: list[str], period: str = "10y") -> pd.DataFrame:
    ...
    if isinstance(data.columns, pd.MultiIndex):
        if "Close" not in data.columns.get_level_values(0):
            raise ValueError("Close価格が取得できませんでした")
...
def portfolio_backtest(
    ...
    if rebalance == "daily":
        rebalance_dates = pd.Series(True, index=returns.index)
    elif rebalance == "monthly":
        periods = returns.index.to_period("M")
        rebalance_dates = pd.Series(periods != periods.shift(1), index=returns.index)
    else:
    ...
    for date in returns.index:
        if rebalance_dates.loc[date] or holdings.sum() == 0:
    ...
    return {
        ...
        "average_turnover_percent": round(float(sum(turnover_values) / len(turnover_values)) * 100, 2),
    }
...
def parse_weights(values: list[str]) -> dict[str, float]:
    ...
    for value in values:
        ticker, sep, weight = value.partition("=")
        if not sep:
